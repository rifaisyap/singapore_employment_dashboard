...
with tab3:
    ...
    by_industry = (latest_year_data
                   .groupby("industry")["employment"].sum()
                   .sort_values(ascending=False)
                   .reset_index())

# Create the bar chart
    st.caption("This chart shows the total number of employed residents for each industry.")
    fig1 = px.bar(
        by_industry, 
        x="industry", 
        y="employment",
    ...
    # Select the top 10 industries from the filtered list above
    top_industries = by_industry["industry"].head(10).tolist()
    
    trends_data = (tab3_filtered[tab3_filtered["industry"].isin(top_industries)]
                   .groupby(["year", "industry"])["employment"].sum()
                   .reset_index())
